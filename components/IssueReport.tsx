...
const IssueReport: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Fetch Transaction Data and Move Orders for the selected period
  ...
  const handleDownloadPPT = async () => {
    ...
    slide1.addText(`ITEM ISSUED SUMMARY OF ${monthName}'${yearStr}`, { 
    ...
    slide2.addText(`DEPT WISE COMMON CONSUMABLES ITEM USED - ${monthName}'${yearStr}`, { 
    ...
    const summaryTotal = ['Grand Total', 
      ...
      allDepartments.reduce((acc, d) => acc + (deptSummaryData[d]?.reqQty || 0), 0).toString(),
                ...
                <table className="w-full text-left border-collapse border border-gray-100">
                  ...
                  <tbody className="text-[11px] font-bold text-gray-600">
                    ...
                    <tr className="bg-[#4472C4] font-black text-white">
                      ...
                      <td className="px-4 py-2 text-center border-r border-white/20">
                        {allDepartments.reduce((acc, d) => acc + (deptSummaryData[d]?.reqQty || 0), 0)}
                      </td>
                      <td className="px-4 py-2 text-center border-r border-white/20">
